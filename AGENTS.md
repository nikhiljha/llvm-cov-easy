...
To find coverage gap line numbers, try `--text` and/or `--json`. Also helpful is `--show-instantiations` to see per-generic-instantiation coverage:

```bash
cargo +nightly llvm-cov nextest --branch --text --show-instantiations 2>&1 | less
```

Look for `^0` annotations which indicate zero executions of a sub-expression. A helpful page to look at if you need help with coverage is the llvm-cov README: https://raw.githubusercontent.com/taiki-e/cargo-llvm-cov/refs/heads/main/README.md
